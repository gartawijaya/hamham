...
class HamsterKombatGame {
	...
	async checkPromoCodes(promoCodes, requiredCount = 5) {
		...
		for (const [type, codes] of Object.entries(promoCodes)) {
			...
			if (codes.length < requiredCount) {
				this.log(`Số lượng mã ${type} không đủ ${requiredCount}. Có ${codes.length} mã.`);
				valid = false;
			}
		}
		if (!valid) {
    ...
    async redeemPromoCodes(dancay, authorization, codes) {
        const successfulCodes = { BIKE: [], TRAIN: [], CUBE: [], MERGE: [], TWERK: [], POLY: [], TRIM: [], RACE: [], CAFE: [] };
        ...
        while (true) {
            for (let i = 0; i < this.authorizationList.length; i++) {
                ...
                if (promo) {
                    ...
                    await this.checkPromoCodes(promoCodes);
                    const selectedCodes = await this.selectCodes(promoCodes);
